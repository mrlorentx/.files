...
> **Community default.** A company skill that explicitly supersedes `skills/golang-naming` skill takes precedence.
...
Many naming convention issues are caught automatically by linters: `revive`, `predeclared`, `misspell`, `errname`. See `skills/golang-linter` skill for configuration and usage.

## Cross-References

- → See `skills/golang-code-style` skill for broader formatting and style decisions
- → See `skills/golang-structs-interfaces` skill for interface naming depth and receiver design
- → See `skills/golang-linter` skill for automated enforcement (revive, predeclared, misspell, errname)
