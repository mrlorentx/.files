...
> **Community default.** A company skill that explicitly supersedes `skills/golang-context` skill takes precedence.
...
- → See the `skills/golang-design-patterns` skill for timeout and resilience patterns

## Enforce with Linters

Many context pitfalls are caught automatically by linters: `govet`, `staticcheck`. → See the `skills/golang-linter` skill for configuration and usage.
