...
> **Community default.** A company skill that explicitly supersedes `skills/golang-design-patterns` skill takes precedence.

# Go Design Patterns & Idioms

Idiomatic Go patterns for production-ready code. For error handling details see the `skills/golang-error-handling` skill; for context propagation see `skills/golang-context` skill; for struct/interface design see `skills/golang-structs-interfaces` skill.
...
12. **Use `strings.Builder`** for concatenation in loops → see `skills/golang-code-style`
...
→ See `skills/golang-structs-interfaces` for the `var _ Interface = (*Type)(nil)` pattern.

## Error Flow Patterns

Error cases MUST be handled first with early return — keep the happy path at minimal indentation. → See `skills/golang-code-style` for the full pattern and examples.
...
Retry logic MUST check `ctx.Err()` between attempts and use exponential/linear backoff via `select` on `ctx.Done()`. Long loops MUST check `ctx.Err()` periodically. → See `skills/golang-context` skill.
...
- → See `skills/golang-data-structures` skill for data structure selection, internals, and container/ packages
- → See `skills/golang-error-handling` skill for error wrapping, sentinel errors, and the single handling rule
- → See `skills/golang-structs-interfaces` skill for interface design and composition
- → See `skills/golang-context` skill for timeout and cancellation patterns
