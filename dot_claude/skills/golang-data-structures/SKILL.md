...
Built-in and standard library data structures: internals, correct usage, and selection guidance. For safety pitfalls (nil maps, append aliasing, defensive copies) see `skills/golang-safety` skill. For string/byte/rune choice see `skills/golang-design-patterns` skill.
...
A slice is a 3-word header: pointer, length, capacity. Multiple slices can share a backing array (→ see `skills/golang-safety` for aliasing traps and the header diagram).
...
Key functions: `Sort`/`SortFunc`, `BinarySearch`, `Contains`, `Compact`, `Grow`. For `Clone`, `Equal`, `DeleteFunc` → see `skills/golang-safety` skill.
...
For `Clone`, `Equal`, sorted iteration → see `skills/golang-safety` skill.
...
- → See `skills/golang-safety` skill for nil map/slice pitfalls, append aliasing, defensive copying, `slices.Clone`/`Equal`
- → See `skills/golang-design-patterns` skill for `string` vs `[]byte` vs `[]rune`, iterators, streaming
- → See `skills/golang-structs-interfaces` skill for struct composition, embedding, and generics vs `any`
- → See `skills/golang-code-style` skill for slice/map initialization style
