...
> **Community default.** A company skill that explicitly supersedes `skills/golang-error-handling` skill takes precedence.
...
- → See `skills/golang-safety` for nil interface trap and nil error comparison pitfalls
- → See `skills/golang-naming` for error naming conventions (ErrNotFound, PathError)
