...
> **Community default.** A company skill that explicitly supersedes `skills/golang-testing` skill takes precedence.
...
Many test best practices are enforced automatically by linters: `thelper`, `paralleltest`, `testifylint`. See the `skills/golang-linter` skill for configuration and usage.

## Cross-References

- -> See `skills/golang-stretchr-testify` skill for detailed testify API (assert, require, mock, suite)
- -> See `skills/golang-linter` skill for testifylint and paralleltest configuration
