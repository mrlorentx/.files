...
local theme_dirs = {
	vim.fn.expand("~/.local/share/omarchy/themes"), -- bundled
	vim.fn.expand("~/.config/omarchy/themes"), -- user-installed
}
...
for _, themes_dir in ipairs(theme_dirs) do
	...
	if handle then
		while true do
			...
			local neovim_lua = themes_dir .. "/" .. theme_name .. "/neovim.lua"
...
-- Map Omarchy theme names to Neovim colorschemes when they don't match the loader defaults.
require("omarchy-theme-loader").setup({
	themes = {
		["void"] = { colorscheme = "catppuccin" },
		["archwave"] = { colorscheme = "tokyonight-moon" },
	},
})
